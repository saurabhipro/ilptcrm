    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('#create-product').click(function () {
                changePageTitle('Create DataList');
                $('#loader-image').show();
                $('#create-product').hide();
                $('#read-products').show();
        ...
        function showProducts() {
            changePageTitle('Search Data Lists');
            $('#page-content').fadeOut('slow', function () {
        ...
        $(document).on('click', '.edit-btn', function () {
            changePageTitle('Update Data List');
            var datalist_id = $(this).closest('td').find('.product-id').text();
            $('#loader-image').show();
            $('#create-product').hide();
            $('#read-products').show();
            $('#page-content').fadeOut('slow', function () {
                $('#page-content').load('update_form.php?datalist_id=' + datalist_id, function () {
                    $('#loader-image').hide();
                    $('#page-content').fadeIn('slow');
                });
            });
...
<div class="container">
    ...
    <div class='page-header'>
        <h1 id='page-title'>Search DataList</h1>
    </div>

    <div class='margin-bottom-1em overflow-hidden'>
        <!-- when clicked, it will show the product's list -->
        <div id='read-products' class='btn btn-primary pull-right display-none'>
            <span class='glyphicon glyphicon-list'></span> Search DataList
        </div>

        <!-- when clicked, it will load the create DataList form -->
        <div id='create-product' class='btn btn-primary pull-right'>
            <span class='glyphicon glyphicon-plus'></span> Create DataList
        </div>
